...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for (n <- ns \\ "a") yield createURL(Some(u), n.toString())
    ...
    for(c <- getURLContent(u); fls <- MonadOps.asFuture(getLinks(c))) yield fls
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    val fus: Seq[Future[Seq[URL]]] = for(u <- us2) yield wget(u)
    val fuse: Seq[Future[Either[Throwable, Seq[URL]]]] = for(fu <- fus) yield MonadOps.sequence(fu)

    Future.sequence(fuse) // Future.sequence converts Seq[Future[X]] to Future[Seq[X]]
  }
